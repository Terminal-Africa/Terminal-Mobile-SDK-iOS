...
class ShipmentsRemote: NetworkService {
    ...
    func createShipment<ParcelM: Codable>(request: CreateShipmentRequest, runCompletionOnUIThread: Bool, completion: @escaping(Result<ShipmentPopulatedWithPackagingData<ParcelM>, Error>) -> Void){
    ...
    /// This function updates a Shipment. Note that shipments that have already been arranged can't be updated.
    /// - Parameters:
    ///   - shipmentId: A unique string used to identify the shipment to update.
